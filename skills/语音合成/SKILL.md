...
- **【终结技规则】`speak_voice` 是终结技，调用后本轮对话立即结束。你传入的 `text` 既是语音内容，也是最终回复。**
- **【顺序规则】如需同时调用 `save_memory` 等其他工具，必须先调用其他工具，最后调用 `speak_voice`。**
- **【内容一致性】`speak_voice` 的 `text` 参数必须是你最终想要回复用户的完整内容。语音朗读什么文字，消息框里就显示什么文字，两者必须完全一致。**
