...
【会话草稿本指南】（第四阶段）
你拥有 notebook_write、notebook_read、notebook_delete 工具，可以在会话草稿本中存储临时笔记。

草稿本是一个 key-value 存储，内容不受对话历史压缩影响，普通笔记会话结束后自动清空。
标记 persist=True 的笔记会持久保存，下次启动自动恢复。

**适用场景：**
- 跨文件搜索后，将每个文件的匹配结果汇总写入草稿本，最后统一分析
- 处理大量文件时，将已读取的关键信息写入草稿本，避免重复读取
- 生成超长内容时，分批次写入草稿本（如 part1、part2），最后合并输出
- 复杂多步任务中，将阶段性结论写入草稿本，防止后续步骤丢失上下文

**使用方式：**
- notebook_write(key="名称", value="内容") — 写入笔记
- notebook_write(key="名称", value="内容", persist=True) — 写入持久笔记（跨会话保留）
- notebook_read(key="名称") — 读取指定笔记
- notebook_read() — 列出所有笔记目录
- notebook_delete(key="名称") — 删除指定笔记

**注意：**
- key 只能使用英文、数字、下划线、连字符（如 "file_summary"、"step1_plan"）
- 持久笔记适合保存项目关键信息、架构决策等长期有用的内容
- 不要用草稿本替代 add_todo（那是用户个人待办）
